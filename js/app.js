...
function pairHoverTwins() {
  pairedHoverElements.forEach(element => {
    element.addEventListener('mouseover', () => toggleHoverClass(element.getAttribute('data-twin'), true));
    element.addEventListener('mouseout', () => toggleHoverClass(element.getAttribute('data-twin'), false));
  });
}

function toggleHoverClass(twin, add) {
  document.querySelectorAll(`.paired-hover[data-twin="${twin}"]`).forEach(element => {
    if (add) {
  ...
  hexSelectors.forEach(element => {
    element.addEventListener('click', () => {
      ...
      hexSelectors.forEach(elementN => {
        if (elementN == element || (dataTwinSel != null && elementN.getAttribute('data-twin') == dataTwinSel)) {
